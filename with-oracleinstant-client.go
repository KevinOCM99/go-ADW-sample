...
func doDBThingsThroughInstantClient(dbParams map[string]string) {
	...
	if val, ok := dbParams["walletLocation"]; ok && val != "" {
		...
		connectString="%s" configDir="%s"
		   `, dbParams["username"], dbParams["password"], dbParams["tns_entry"], dbParams["walletLocation"]))
	}
	if val, ok := dbParams["walletLocation"]; !ok || val == "" {
...
func doDBThingsThroughInstantClientArray(dbParams map[string]string) {
	var db *sql.DB
	var err error
	if val, ok := dbParams["walletLocation"]; ok && val != "" {
		db, err = sql.Open("godror", fmt.Sprintf(`user="%s" password="%s"
